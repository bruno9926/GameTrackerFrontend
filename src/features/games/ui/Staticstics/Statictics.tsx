...
const GamesFinished = ({ gamesFinished }: { gamesFinished: number }) => {
    return (
        <StaticsticCard variant="primary">
            <span className="text-xl md:text-3xl">You have <span className="font-bold not-italic">finished {gamesFinished} games</span> this year!</span>
        </StaticsticCard>
    )
}
...
const GamesInProgress = ({ gamesInProgress }: { gamesInProgress: number }) => {
    return (
        <StaticsticCard variant="secondary">
            <span className="text-xl md:text-3xl">You have <span className="font-bold not-italic"> {gamesInProgress} games in progress</span>!</span>
        </StaticsticCard>
    )
}

const StaticsticCard = ({ children, variant = 'primary' }: { children: React.ReactNode, variant?: 'primary' | 'secondary' }) => {
    const styles = cva('card w-45 md:w-60 p-6 border-0 text-accent-foreground shadow-lg italic shrink-0 hover:scale-105 transition-transform', {
        variants: {
            variant: {
